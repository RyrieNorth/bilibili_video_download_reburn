...
from .utils import load_config, load_cookie
...
def main(video_bv):
    try:
        video_data, is_single_part = get_video_data(
            video_bv, url=config["url"]["convert_cid"], header=config["basic_headers"]
        )
        video_info = get_video_info(
            video_bv, video_data[0]["cid"] if not is_single_part else video_data["cid"]
        )
        ...
        if is_single_part:
            ...
            video_url, audio_url = get_video_download_info(
                video_bv, video_data, selected_quality_id
            )
            ...
            for video in video_data:
                ...
                video_url, audio_url = get_video_download_info(
                    video_bv, video, selected_quality_id
                )
